...
class SoundFrontEnd
{
	 ...
	 * Default keys: + (and numpad +). Set to [] to deactivate.
	 */
	 ...
	 * Default keys: 0 (and numpad 0). Set to [] to deactivate.
	 */
	public var masterMuteKeys:Array<FlxKey> = [ZERO, NUMPADZERO];

	/**
	 * The key modifiers to use for Master volume
	 * Default key: SHIFT. Set to [] to deactivate
	 */
	public var masterModifier:Array<FlxKey> = null;
	 ...
	 */
	public var soundModifier:Array<FlxKey> = [SHIFT];

	/**
	 * The key codes used to increase Music volume (see FlxG.keys for the keys available).
	 * Default keys: PAGEUP (and numpad 9). Set to [] to deactivate.
	 */
	public var musicVolumeUpKeys:Array<FlxKey> = [PLUS, NUMPADPLUS];

	/**
	 * The keys to decrease Music volume (see FlxG.keys for the keys available).
	 * Default keys: PAGEDOWN (and numpad 3). Set to [] to deactivate.
	 */
	public var musicVolumeDownKeys:Array<FlxKey> = [MINUS, NUMPADMINUS];

	/**
	 * The keys used to mute / unmute the Music volume (see FlxG.keys for the keys available).
	 * Default keys: DELETE (and numpad Period). Set to [] to deactivate.
	 */
	public var musicMuteKeys:Array<FlxKey> = [ZERO, NUMPADZERO];

	/**
	 * The key modifier to use for Sound volume
	 * Default: deactivated. Set to a key like SHIFT or CONTROL to activate
	 */
	public var musicModifier:Array<FlxKey> = [CONTROL];
	#end
	...
	public function toggleMuted(WhichVolume:Int = VOL_MASTER):Void
	{
		...
		{
			case VOL_MASTER:
				defaultSoundGroup.muted = defaultMusicGroup.muted = muted = !muted;
				if (volumeHandler != null)
				{
					volumeHandler(muted ? 0 : volume, VOL_SOUND);
					volumeHandler(muted ? 0 : volume, VOL_MUSIC);
				}

			case VOL_SOUND:
				defaultSoundGroup.muted = !defaultSoundGroup.muted;
				muted = defaultSoundGroup.muted && defaultMusicGroup.muted;
				if (volumeHandler != null)
				{
					volumeHandler(muted ? 0 : volume, VOL_SOUND);
				}
			...
			case VOL_MUSIC:
				defaultMusicGroup.muted = !defaultMusicGroup.muted;
				muted = defaultSoundGroup.muted && defaultMusicGroup.muted;
				if (volumeHandler != null)
				{
					volumeHandler(muted ? 0 : volume, VOL_MUSIC);
		...
		showSoundTray(true, WhichVolume == VOL_MUSIC);
	}
	...
	public function changeVolume(Amount:Float, WhichVolume:Int = VOL_MASTER):Void
	{
		switch (WhichVolume)
		{
			case VOL_MASTER:
				volume += Amount;
				defaultSoundGroup.muted = defaultMusicGroup.muted = muted = false;

			case VOL_SOUND:
				defaultSoundGroup.volume += Amount;
				muted = defaultSoundGroup.muted = false;

			case VOL_MUSIC:
				defaultMusicGroup.volume += Amount;
				muted = defaultMusicGroup.muted = false;
		}

		updataSavePrefs();

		showSoundTray(Amount > 0, WhichVolume == VOL_MUSIC);
	}
	...
	function update(elapsed:Float):Void
	{
		...
		if (FlxG.keys.anyJustReleased(masterMuteKeys)
			&& ((masterModifier == null && !FlxG.keys.anyPressed(soundModifier.concat(musicModifier)))
				|| FlxG.keys.anyPressed(masterModifier)))
			...
			&& ((masterModifier == null && !FlxG.keys.anyPressed(soundModifier.concat(musicModifier)))
				|| FlxG.keys.anyPressed(masterModifier)))
			changeVolume(-0.1, VOL_MASTER);
		else if (FlxG.keys.anyJustReleased(soundMuteKeys)
			&& ((soundModifier == null && !FlxG.keys.anyPressed(masterModifier.concat(musicModifier)))
				|| FlxG.keys.anyPressed(soundModifier)))
			toggleMuted(VOL_SOUND);
		else if (FlxG.keys.anyJustReleased(soundVolumeUpKeys)
			&& ((soundModifier == null && !FlxG.keys.anyPressed(masterModifier.concat(musicModifier)))
				|| FlxG.keys.anyPressed(soundModifier)))
			changeVolume(0.1, VOL_SOUND);
		else if (FlxG.keys.anyJustReleased(soundVolumeDownKeys)
			&& ((soundModifier == null && !FlxG.keys.anyPressed(masterModifier.concat(musicModifier)))
				|| FlxG.keys.anyPressed(soundModifier)))
			changeVolume(-0.1, VOL_SOUND);
		else if (FlxG.keys.anyJustReleased(musicMuteKeys)
			&& ((musicModifier == null && !FlxG.keys.anyPressed(masterModifier.concat(soundModifier)))
				|| FlxG.keys.anyPressed(musicModifier)))
			toggleMuted(VOL_MUSIC);
		else if (FlxG.keys.anyJustReleased(musicVolumeUpKeys)
			&& ((musicModifier == null && !FlxG.keys.anyPressed(masterModifier.concat(soundModifier)))
				|| FlxG.keys.anyPressed(musicModifier)))
			changeVolume(0.1, VOL_MUSIC);
		else if (FlxG.keys.anyJustReleased(musicVolumeDownKeys)
			&& ((musicModifier == null && !FlxG.keys.anyPressed(masterModifier.concat(soundModifier)))
				|| FlxG.keys.anyPressed(musicModifier)))
			changeVolume(-0.1, VOL_MUSIC);
		#else
	...
	/**
	 * Saves volume statuses
	**/
	function updataSavePrefs():Void
	{
		#if SPLIT_SOUND_TRAY
		FlxG.save.data.volume = volume;

		FlxG.save.data.soundVolume = defaultSoundGroup.volume;
		FlxG.save.data.musicVolume = defaultMusicGroup.volume;
		FlxG.save.data.muted = muted;
		FlxG.save.data.soundmute = defaultSoundGroup.muted;
		FlxG.save.data.musicmute = defaultMusicGroup.muted;
		FlxG.save.flush();
		#end
	}
